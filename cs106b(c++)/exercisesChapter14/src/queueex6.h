 ...
 */
template <typename ValueType>
ValueType QueueEx6<ValueType>::dequeue() {
    if (count == 0) {
...
template <typename ValueType>
void QueueEx6<ValueType>::enqueue(const ValueType& value) {
    if (count >= capacity - 1) {
        expandRingBufferCapacity();
    }
    //ringBuffer[tail] = value;
    //tail = (tail + 1) % capacity;
    ringBuffer[(head + count) % capacity] = value;
    count++;
}


template <typename ValueType>
bool QueueEx6<ValueType>::isEmpty() const {
    return count == 0;
}

template <typename ValueType>
const ValueType& QueueEx6<ValueType>::peek() const {
    if (count == 0) {
...
template <typename ValueType>
int QueueEx6<ValueType>::size() const {
    return count;
}
 ...
 */
template <typename ValueType>
void QueueEx6<ValueType>::expandRingBufferCapacity() {
