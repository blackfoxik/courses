...
int shortestPathLength(Grid<int> & maze);
bool shortestPathLengthSearcher(int row, int col, Grid<int> & maze, Vector<int> & lenghtsOfPaths, int curLenght);
...
void exercise1() {
    ...
    printMaze(maze);
    int minLenght = shortestPathLength(maze);
...
int shortestPathLength(Grid<int> & maze) {
    Vector<int> lenghtsOfPaths;
    shortestPathLengthSearcher(1, 1, maze, lenghtsOfPaths, 0);
    maze[1][1] = 'S'; // replace start, which is removed during solving stage
    int minLenght = 0;
    cout << "count of paths is : " << lenghtsOfPaths.size() << endl;
    if (lenghtsOfPaths.size() > 0) {
       minLenght = lenghtsOfPaths.get(0);
       for (int i = 0; i < lenghtsOfPaths.size(); i++) {
           int curLenght = lenghtsOfPaths.get(i);
           cout << "lenght of cur path is : " << lenghtsOfPaths.get(i) << endl;
           if (lenghtsOfPaths.get(i) < minLenght ) {
               minLenght = lenghtsOfPaths.get(i);
           }
       }
    } else {
       minLenght = -1;
...
bool shortestPathLengthSearcher(int row, int col, Grid<int> & maze, Vector<int> & lenghtsOfPaths, int curLenght) {
   ...
   // printMaze(maze);
    ...
    // north
    if (shortestPathLengthSearcher(row-1,col,maze, lenghtsOfPaths, curLenght)) {
        //return true;
    }

    // east
    if (shortestPathLengthSearcher(row,col+1,maze, lenghtsOfPaths, curLenght)) {
        //return true;
    }
